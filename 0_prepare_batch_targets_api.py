"""
Pipeline Stage 0 (API Variant): Streamlined High-Performance Batch Target Generator.

Targets ONLY API MLS sources that are active in both Staging and Production.
...
REST_PROTOCOL_ID = int(os.getenv("REST_PROTOCOL_ID", 3487705513625387016))
OAUTH2_PROTOCOL_ID = int(os.getenv("OAUTH2_PROTOCOL_ID", 3487705513674153992))
...
def generate_batch(mls_limit: int, auto_approve: bool = False):
    ...
    try:
        ...
        with SSHTunnelForwarder(
            ...
            with psycopg2.connect(
                ...
                with conn.cursor() as cur:
                    ...
                    """
                    cur.execute(query_1, (REST_PROTOCOL_ID, OAUTH2_PROTOCOL_ID, allowed_prod_mls_list, excluded_mls_list or [-1]))
                    ...
                    if not selected_mls_set:
                        logger.info("🎉 No unmigrated legacy rules found for active API sources!")
                        return
        ...
        with open(api_targets_csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=csv_headers)
            writer.writeheader()
