...
5. Fetches metadata and generates 'rets_mls.txt', 'rets_rules.txt', and 'batch_mls_targets_rets.csv'.

Usage:
    python 0_prepare_batch_targets_rets.py           # Uses default limit (5 MLSs)
    python 0_prepare_batch_targets_rets.py --limit 1 # Picks targets across 1 distinct MLS
"""
    ...
    try:
        ...
        if not auto_approve:
            ...
            if not prompt_user_confirmation(prompt_msg):
                logger.warning("Reverse promotion aborted by operator. Stopping Stage 0 RETS target generation.")
                sys.exit(0)
        ...
        if rev_result.returncode != 0:
            logger.error("❌ Reverse promotion failed! Aborting RETS target generation.")
            return
