"""
Pipeline Stage 0: Automated Batch Target Selector & Reverse Promotion Invoker (RETS Variant).

Selects unmigrated RETS MLS sources (filtered by optional assigned MLS list),
...
def prepare_rets_batch(limit: int, auto_approve: bool, assigned_mls_input: str | None = None) -> None:
            ...
            with psycopg2.connect(
                ...
                with conn.cursor() as cur:
                    ...
                    for m_id, m_id_str, _, _ in cur.fetchall():
                        ...
                        # Filter against ticket assignment if provided
                ...
                if not selected_mls_ids:
                    logger.info("ℹ️ No eligible unmigrated RETS MLS sources match the selection criteria.")
                    return

                logger.info(f"Selected target Production-Verified RETS MLS IDs ({len(selected_mls_ids)}): {selected_mls_ids}")
                ...
                rets_mls_file = temp_dir / "rets_mls.txt"
                rets_rules_file = temp_dir / "rets_rules.txt"
                batch_csv_file = temp_dir / "batch_mls_targets_rets.csv"
                master_csv_file = temp_dir / "batch_mls_targets.csv"

                rets_mls_file.write_text("\n".join(str(x) for x in selected_mls_ids) + "\n", encoding="utf-8")
                rets_rules_file.write_text("\n".join(sorted(list(target_rules))) + "\n", encoding="utf-8")
                ...
                logger.info(f"Generated: {rets_mls_file}")
                logger.info(f"Generated: {rets_rules_file}")
                logger.info(f"Generated: {batch_csv_file}")
                logger.info(f"Updated Working Target: {master_csv_file}")
                logger.info("============================================================")

    except Exception as e:
        logger.error(f"❌ Failed during Stage 0 RETS batch preparation: {e}", exc_info=True)
        sys.exit(1)


def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 0: Prepare Target RETS MLS Batch")
    parser.add_argument("-l", "--limit", type=int, default=4, help="Maximum number of target RETS MLS sources to select")
    ...
    prepare_rets_batch(limit=args.limit, auto_approve=args.auto_approve, assigned_mls_input=args.assigned_mls)
