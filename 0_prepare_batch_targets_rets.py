"""
Pipeline Stage 0 (RETS Variant): Two-Step High-Performance Batch Target Generator.

Targets ONLY ACTIVE RETS MLS sources (mls_status_id = 2 & RETS Integration protocol).
Generates 'rets_mls.txt', 'rets_rules.txt', and 'batch_mls_targets_rets.csv' based on
a configurable MLS target limit (--limit / -l).

Sorting Hierarchy:
    1. Rules ranked by distinct active RETS MLS count (ASCENDING)
...
# Exact RETS Integration Protocol ID
RETS_PROTOCOL_ID = 3487705513709191176
...
def generate_batch(mls_limit: int):
    """
    Selects up to `mls_limit` distinct active RETS MLS sources starting from the
    lowest-impact legacy rules, and writes rets_mls.txt, rets_rules.txt, and batch_mls_targets_rets.csv.
    """
    ...
    logger.info(f"Target MLS batch size limit: {mls_limit} (RETS Protocol: {RETS_PROTOCOL_ID})")
        ...
        with SSHTunnelForwarder(
            ...
            with psycopg2.connect(
                ...
                with conn.cursor() as cur:
                    # -------------------------------------------------------------
                    # QUERY 1: Map legacy rules to active RETS MLS IDs, ordered by rule impact
                    # -------------------------------------------------------------
                    logger.info("Step 1: Identifying and ranking target active RETS MLS sources...")
                    query_1 = """
                        WITH rule_counts AS (
                            SELECT 
                    ...
                    """
                    cur.execute(query_1, (RETS_PROTOCOL_ID, RETS_PROTOCOL_ID))
                    ...
                    if not selected_mls_set:
                        logger.info("🎉 No unmigrated legacy rules found for unprocessed active RETS sources!")
                        return

                    target_mls_list = sorted(list(selected_mls_set))
                    logger.info(f"Selected target RETS MLS IDs ({len(target_mls_list)}): {target_mls_list}")

                    # -------------------------------------------------------------
                    # QUERY 2: Fetch detailed metadata for ALL legacy rules on selected RETS MLSs
                    # -------------------------------------------------------------
